...
class Cluster(object):
    ...
    # walletd [True|False] Is remvault running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
        ...
        """Cluster container.
        walletd [True|False] Is wallet remvault running. If not load the wallet plugin
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        extraNodeosArgs: string of arguments to pass through to each remnode instance (via --remnode flag on launcher)
        ...
        specificExtraNodeosArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-remnode flags on launcher), example: { "5" : "--plugin eosio::test_control_api_plugin" }
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on remnode default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        if specificExtraNodeosArgs is not None:
            ...
            for nodeNum,arg in specificExtraNodeosArgs.items():
                ...
                cmdArr.append("--specific-remnode")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-remnode")
                cmdArr.append(path)
    ...
    def publishContract(self, node, contract, account):
        ...
        Utils.Print("Publishing %s contract" % (contract))
        return node.publishContract(account, contractDir, wasmFile, abiFile, waitForTransBlock=True)
    ...
    def setDefaultProducers(self, node, total_nodes, number_of_producers, producer_keys):
        if number_of_producers == -1:
            ...
            counts = dict.fromkeys(range(total_nodes), 0) #initialize node prods count to 0
            setProdsStr = '{"schedule": ['
            prodNames = []
            for _, keys in producer_keys.items():
                if counts[keys["node"]] >= number_of_producers:
                    continue
                if len( prodNames ) > 0:
                    setProdsStr += ','
    ...
    def issueTokens(self, node, biosAccount, tokenContract, totalProducers = 0):
        Utils.Print("push create action to %s contract" % (tokenContract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (biosAccount.name, CORE_SYMBOL)
        ...
        Utils.Print("push issue action to %s contract" % (tokenContract))
        action = "issue"
        initialAmount = "1000000000.0000 {0}".format(CORE_SYMBOL)
        data="{\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"initial issue\"}" % (biosAccount.name, initialAmount)
        ...
        timeout = .5 * 12 * 2 * totalProducers + 60
        if not node.waitForTransFinalization(transId, timeout=timeout):
            Utils.Print("ERROR: Failed to validate transaction %s got rolled into a finalized block on server port %d." % (transId, node.port))
            return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        """
        Create 'prodCount' init accounts and deposits 10000000000 SYS in each.
        If prodCount is -1 will initialize all possible producers.
        Ensure nodes are inter-connected prior to this call.
        One way to validate this will be to check if every node has block 1.
        """
        ...
        for name, keys in producerKeys.items():
            initx = Account(
                name, keys["private"], keys["public"], keys["private"], keys["public"]
            )
        ...
        for account in systemAccounts:
            Utils.Print("Creating %s account" % account.name)
        ...
        if self.issueTokens(biosNode, eosioAccount, tokenAccountName, len(producerKeys)) is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (tokenAccountName, name))
                return None
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster remnode instances. allInstances will kill all remnode instances running on the system."""
    ...
    def bounce(self, nodes, silent=True):
        """Bounces remnode instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down remnode instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
