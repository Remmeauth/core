...
class Cluster(object):
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        loadSystemContract: indicate whether the remme.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        ...
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "contracts/contracts/remme.bios"
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"remme\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init eosio transfer")
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission remme@active"
                    myTrans=biosNode.pushMessage("remme", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("remme", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "remme"
        ...
        eosioTokenAccount.name="remme.token"
        ...
        eosioRamAccount.name="remme.ram"
        ...
        eosioRamfeeAccount.name="remme.ramfee"
        ...
        eosioStakeAccount.name="remme.stake"
