...
issue_tokens( TESTER& t, account_name issuer, account_name to, const asset& amount,
              std::string memo = "", account_name token_contract = N(rem.token) )
{
...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   push_action(N(rem.token), N(create), N(rem.token), act );
   ...
   eosio::chain_apis::read_only::get_table_by_scope_params param{N(rem.token), N(accounts), "inita", "", 10};
   eosio::chain_apis::read_only::get_table_by_scope_result result = plugin.read_only::get_table_by_scope(param);

   //TODO: why we had 4 before? As I see we do issue first to system rem account and transfer to speficic accounts, so
   //we should have 5 records.
   BOOST_REQUIRE_EQUAL(5u, result.rows.size());
   BOOST_REQUIRE_EQUAL("", result.more);
   if (result.rows.size() >= 4) {
      BOOST_REQUIRE_EQUAL(name(N(rem.token)), result.rows[0].code);
      BOOST_REQUIRE_EQUAL(name(N(inita)), result.rows[0].scope);
      BOOST_REQUIRE_EQUAL(name(N(accounts)), result.rows[0].table);
      BOOST_REQUIRE_EQUAL(name(N(rem)), result.rows[0].payer);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   if (result.rows.size() >= 4) {
      ...
      BOOST_REQUIRE_EQUAL("rem", result.rows[0]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("rem", result.rows[1]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("rem", result.rows[2]["payer"].as_string());
      BOOST_REQUIRE_EQUAL("rem", result.rows[3]["payer"].as_string());
   }
   p.show_payer = false;
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   auto bidname = [this]( const account_name& bidder, const account_name& newname, const asset& bid ) {
      return push_action( N(rem), N(bidname), bidder, fc::mutable_variant_object()
                          ("bidder",  bidder)
                          ("newname", newname)
                          ("bid", bid)
                          );
