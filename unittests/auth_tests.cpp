...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   chain.link_authority(name("alice"), name("rem"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("rem"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with eosio reqauth
   chain.unlink_authority(name("alice"), name("rem"), name("reqauth"));
   ...
   chain.link_authority(name("alice"), name("rem"), name("scud"));
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority(name("alice"), name("rem"), name("first"), name("reqauth"));
...
try {
   ...
   chain.create_account(name("rem.test1"));

   // Creating account with rem. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("rem.test2"), name("joe")), action_validate_exception,
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("rem"), name("rem.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("rem"), name("rem.any"), name("reqauth") );
