...
read_only::get_voters_result read_only::get_voters( const read_only::get_voters_params& p ) const {
   ...
   try{
      ...
      const auto lower = name{p.lower_bound};
      auto it = kv_index.lower_bound(boost::make_tuple(table_id->id, lower.to_uint64_t()));
      ...
      for( ; it != kv_index.end(); ++it ) {
         ...
         if (p.json)
            result.rows.emplace_back( abis.binary_to_variant( abis.get_table_type(N(voters)), data, abi_serializer::create_yield_function( abi_serializer_max_time ), shorten_abi_errors ) );
         else
            result.rows.emplace_back(fc::variant(data));
      }
   }
