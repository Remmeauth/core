...
namespace eosiosystem {
   ...
   void system_contract::torewards( const name& payer, const asset& amount ) {
      require_auth( payer );
      const auto to_per_stake_pay = amount.amount * _gstate5.per_stake_share;
      const auto to_per_vote_pay  = share_pervote_reward_between_producers(amount.amount * _gstate5.per_vote_share);
