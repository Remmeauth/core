...
namespace eosiosystem {
   ...
   double system_contract::stake2vote( int64_t staked, time_point locked_stake_period ) const {
      ...
      const auto weeks_to_mature = fmax( ((locked_stake_period - current_time_point()) - eosio::minutes(7)).count() / eosio::minutes(7).count(), 0.0 );
      const auto rem_weight = 1.0 - (weeks_to_mature) / (_gremstate.stake_lock_period.count() / eosio::minutes(7).count());
      uint32_t seconds_per_minute = 60;
      const double eos_weight = std::pow( 2, int64_t((current_time_point().sec_since_epoch() - (block_timestamp::block_timestamp_epoch / 1000)) / (seconds_per_minute * 7)) / double(52) );
