...
namespace eosiosystem {
   ...
   double system_contract::stake2vote( int64_t staked, time_point locked_stake_period ) const {
      ...
      const double days_to_mature = fmax( (locked_stake_period - current_time_point()).to_seconds() / seconds_per_day, 0.0 );
      const auto rem_weight = 1.0 - days_to_mature / (_gstate.stake_lock_period.to_seconds() / seconds_per_day);
