...
namespace eosiosystem {
                                       ...
                                       const asset& unstake_quantity)
   {
      ...
      if ( voter.last_undelegate_time <= voter.stake_lock_time + _gremstate.stake_unlock_period ) {
         ...
         check( ct - voter.last_undelegate_time > eosio::seconds(1), "already undelegated within past day" );

         const auto unclaimed_days = voter.last_undelegate_time.time_since_epoch().count() ? (ct - voter.last_undelegate_time).count() / eosio::seconds( 1 ).count()
                                                                                             : 1;

         const auto unlock_period_in_days = _gremstate.stake_unlock_period.count() / eosio::seconds( 1 ).count();
