...
namespace eosio {
   ...
   void attribute::confirm( const name& owner, const name& issuer, const name& attribute_name )
   {
      ...
      check( attr.attributes.count(attribute_name) == 1, "account does not have this attribute set by provided issuer" );
      attributes.modify( attr, same_payer, [&]( auto& attr ) {
         const auto attr_it = attr.attributes.find(attribute_name);
         attr_it->second.data.swap(attr_it->second.pending);
         attr_it->second.pending.clear();
      });
   }
