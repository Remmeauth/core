...
public:
   eosio_msig_tester() {
      create_accounts( { N(remme.msig), N(remme.stake), N(remme.ram), N(remme.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(remme.msig), contracts::msig_wasm() );
      set_abi( N(remme.msig), contracts::msig_abi().data() );

      produce_blocks();
      const auto& accnt = control->db().get<account_object,by_name>( N(remme.msig) );
      abi_def abi;
   ...
   transaction_trace_ptr create_account_with_resources( account_name a, account_name creator, asset ramfunds, bool multisig,
      ...
      trx.actions.emplace_back( get_action( N(remme), N(buyram), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("payer", creator)
                                            ("receiver", a)
                                            ("quant", ramfunds) )
      ...
      trx.actions.emplace_back( get_action( N(remme), N(delegatebw), vector<permission_level>{{creator,config::active_name}},
                                            mvo()
                                            ("from", creator)
                                            ("receiver", a)
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(remme.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const string& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(remme.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(remme.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(remme.msig), name, accounts, data );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, eosio_msig_tester ) try {
   ...
   // required to set up the link between (eosio active) and (remme.prods active)
   //
   //                  eosio active
   //                       |
   //             remme.prods active (2/3 threshold)
   //             /         |        \             <--- implicitly updated in onblock action
   // alice active     bob active   carol active

   set_authority(N(remme), "active", authority(1,
      vector<key_weight>{{get_private_key("remme", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(remme.prods), config::active_name}, 1}}), "owner",
      { { N(remme), "active" } }, { get_private_key( N(remme), "active" ) });

   set_producers( {N(alice),N(bob),N(carol)} );
   produce_blocks(50);

   create_accounts( { N(remme.token) } );
   set_code( N(remme.token), contracts::token_wasm() );
   set_abi( N(remme.token), contracts::token_abi().data() );

   create_currency( N(remme.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   ...
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"),
                        get_balance("remme") + get_balance("remme.ramfee") + get_balance("remme.stake") + get_balance("remme.ram") );
   ...
   produce_blocks();
   create_account_with_resources( N(alice1111111), N(remme), core_sym::from_string("1.0000"), false );
   create_account_with_resources( N(bob111111111), N(remme), core_sym::from_string("0.4500"), false );
   create_account_with_resources( N(carol1111111), N(remme), core_sym::from_string("1.0000"), false );
   ...
   vector<permission_level> action_perm = {{N(remme), config::active_name}};
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), N(remme), core_sym::from_string("1.0000"), false ),
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, eosio_msig_tester ) try {
   ...
   // set up the link between (eosio active) and (remme.prods active)
   set_authority(N(remme), "active", authority(1,
      vector<key_weight>{{get_private_key("remme", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(remme.prods), config::active_name}, 1}}), "owner",
      { { N(remme), "active" } }, { get_private_key( N(remme), "active" ) });
   ...
   create_currency( N(remme.token), config::system_account_name, core_sym::from_string("10000000000.0000") );
   issue(config::system_account_name, core_sym::from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_sym::from_string("1000000000.0000"), get_balance( "remme" ) );
...
BOOST_FIXTURE_TEST_CASE( approve_execute_old, eosio_msig_tester ) try {
   set_code( N(remme.msig), contracts::util::msig_wasm_old() );
   set_abi( N(remme.msig), contracts::util::msig_abi_old().data() );
   produce_blocks();

   //propose with old version of remme.msig
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
   push_action( N(alice), N(propose), mvo()
                  ("proposer",      "alice")
                  ("proposal_name", "first")
