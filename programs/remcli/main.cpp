...
chain::action create_delegate(const name& from, const name& receiver, const asset& stake_quantity, bool transfer) {
   fc::variant act_payload = fc::mutable_variant_object()
         ("from", from.to_string())
         ("receiver", receiver.to_string())
         ("stake_quantity", stake_quantity.to_string())
         ("transfer", transfer);
...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->add_flag("--add-code", add_code, localized("[code] add '${code}' permission to specified permission authority", ("code", name(config::rem_code_name))));
      permissions->add_flag("--remove-code", remove_code, localized("[code] remove '${code}' permission from specified permission authority", ("code", name(config::rem_code_name))));
      ...
      permissions->callback([this] {
         ...
         if ( need_parent || need_auth ) {
            ...
            if ( need_auth ) {
               ...
               auto code_name = config::rem_code_name;
...
void ensure_remvault_running(CLI::App* app) {
    if (no_auto_remvault)
        return;
    // get, version, net, convert do not require remvault
...
struct create_account_subcommand {
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
         createAccount->add_option("--stake", stake_quantity,
                                   (localized("The amount of tokens delegated for resources")))->required();
      ...
      createAccount->callback([this] {
            ...
            if (!simple) {
               auto quantity = to_asset(stake_quantity);
               if ( quantity.get_amount() != 0 ) {
                  action delegate = create_delegate( name(creator), name(account_name), quantity, transfer);
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_quantity", stake_quantity, localized("The amount of tokens to stake for resources"))->required();
      ...
      delegate_bandwidth->callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ...
                  ("stake_quantity", to_asset(stake_quantity))
                  ("transfer", transfer);
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_quantity", unstake_quantity, localized("The amount of tokens to undelegate for resources"))->required();
      ...
      undelegate_bandwidth->callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
                  ("from", from_str)
                  ("receiver", receiver_str)
                  ("unstake_quantity", to_asset(unstake_quantity));
         auto accountPermissions = get_account_permissions(tx_permission, {name(from_str), config::active_name});
...
int main( int argc, char** argv ) {
   ...
   app.parse_complete_callback([&app]{ ensure_remvault_running(&app);});
   ...
   string getAttrName;
   auto getAttribute = get->add_subcommand("attribute", localized("Retrieve attribute information"));
   ...
   getAttribute->callback([&] {
      auto result = call(get_table_func, fc::mutable_variant_object("json", true)
         ...
         ("table", "attrinfo")
         ("lower_bound", name(getAttrName).to_uint64_t())
         ("upper_bound", name(getAttrName).to_uint64_t() + 1)
         ("limit", 1)
      );
      auto res = result.as<eosio::chain_apis::read_only::get_table_rows_result>();
      ...
      index_key <<= 64;
      index_key |= name(getAttrIssuer).to_uint64_t();
   ...
   */
   ...
   string setAttrName;
   string attrValue;
   auto setAttribute = setSubcommand->add_subcommand("attribute", localized("Set attribute information"));
   ...
   setAttribute->callback([&] {
      auto result = call(get_table_func, fc::mutable_variant_object("json", true)
         ...
         ("scope", name(config::attribute_account_name))
         ("table", "attrinfo")
         ("lower_bound", name(setAttrName).to_uint64_t())
         ("upper_bound", name(setAttrName).to_uint64_t() + 1)
         ("limit", 1)
      );
      auto res = result.as<eosio::chain_apis::read_only::get_table_rows_result>();
      ...
      send_actions({ create_setattr(name(setAttrIssuer), name(setAttrReceiver), name(setAttrName), bytes) });
   });
...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(rem.msig), N(propose), variant_to_bin( N(rem.msig), N(propose), args ) }});
   });
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, N(rem.msig), name(action), variant_to_bin( N(rem.msig), name(action), args ) }});
   };
   ...
   invalidate->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(rem.msig), N(invalidate), variant_to_bin( N(rem.msig), N(invalidate), args ) }});
   });
   ...
   cancel->callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, N(rem.msig), N(cancel), variant_to_bin( N(rem.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(rem.msig), N(exec), variant_to_bin( N(rem.msig), N(exec), args ) }});
      }
   );
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send rem.system contract action to the blockchain."));
