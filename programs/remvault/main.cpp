...
int main(int argc, char** argv)
{
   try {
      ...
      http_plugin::set_defaults({
         .default_unix_socket_path = remvault::config::key_store_executable_name + ".sock",
         .default_http_port = 0
      });
      ...
      http.add_handler("/v1/" + remvault::config::key_store_executable_name + "/stop", [&a=app()](string, string, url_response_callback cb) { cb(200, fc::variant(fc::variant_object())); a.quit(); } );
      app().startup();
      app().exec();
   } catch (const fc::exception& e) {
      elog("${e}", ("e",e.to_detail_string()));
